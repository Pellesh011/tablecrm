...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["created_by"], ["tg_accounts.id"], name="fk_tg_bot_bills_created_by"
        ),  # Name the foreign key
        sa.ForeignKeyConstraint(
            ["tochka_bank_account_id"],
            ["tochka_bank_accounts.id"],
            name="fk_tg_bot_bills_tochka_bank_account_id",
        ),  # Name the foreign key
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("NEW", "APPROVED", "CANCELED", name="tgbillapprovestatus"),
            nullable=False,
        ),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        ...
        sa.PrimaryKeyConstraint(
            "id", name="pk_tg_bot_bill_approvers"
        ),  # Name the primary key
        sa.ForeignKeyConstraint(
            ["approver_id"],
            ["tg_accounts.id"],
            name="fk_tg_bot_bill_approvers_approver_id",
        ),  # Name the foreign key
        sa.ForeignKeyConstraint(
            ["bill_id"], ["tg_bot_bills.id"], name="fk_tg_bot_bill_approvers_bill_id"
        ),  # Name the foreign key
    )
...
def downgrade() -> None:
    op.drop_constraint(
        "fk_tg_bot_bill_approvers_bill_id", "tg_bot_bill_approvers", type_="foreignkey"
    )
    op.drop_constraint(
        "fk_tg_bot_bill_approvers_approver_id",
        "tg_bot_bill_approvers",
        type_="foreignkey",
    )
    op.drop_constraint(
        "pk_tg_bot_bill_approvers", "tg_bot_bill_approvers", type_="primary"
    )
    op.drop_table("tg_bot_bill_approvers")

    op.drop_constraint(
        "fk_tg_bot_bills_tochka_bank_account_id", "tg_bot_bills", type_="foreignkey"
    )
